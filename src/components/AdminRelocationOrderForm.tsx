...
interface AdminRelocationOrderFormProps {
  eligibleProducts: { id: string; title: string }[];
}

export function AdminRelocationOrderForm({ eligibleProducts }: AdminRelocationOrderFormProps) {
  ...
  const [success, setSuccess] = useState<{ orderId: number; jobId: number } | null>(null);
  ...
  function resetForRepeat() {
    ...
    setThemes([]);
    setMotivations([]);
    setKnowsCities(false);
    setDestinationCities([null]);
    setSuccess(null);
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!productId) {
      setError('Pick which product this is.');
      return;
    }
    if (!customerEmail.trim()) {
      setError("Add the client's email — that's where the finished report gets sent.");
      return;
    }
    if (!client.trim()) {
      setError('Tell us what name to use in the reading.');
      return;
    }
    if (!birthtime) {
      setError('Birth time is needed to calculate houses and relocation lines accurately.');
      return;
    }
    if (!selectedCity) {
      setError('Add the place of birth.');
      return;
    }
    if (themes.length === 0) {
      setError('Pick at least one theme for this reading to focus on.');
      return;
    }
    ...
    try {
      const res = await fetch('/api/admin/relocation-order', {
        method: 'POST',
      ...
      if (!res.ok) {
        throw new Error(data?.error || 'Something went wrong creating this order.');
      }

      setSuccess({ orderId: data.orderId, jobId: data.jobId });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Something went wrong creating this order.');
    } finally {
  ...
  if (success) {
    return (
      <div className="rounded-lg border border-[#2D2640]/10 bg-white p-8 text-center">
        <p className="text-[#2D2640] text-lg mb-2">Generation started.</p>
        <p className="text-[#655E78] text-sm mb-6">
          Order #{success.orderId} · Job #{success.jobId}. The report will email itself to the client when it&apos;s ready
          — no mailing-list subscription was added.
        </p>
        <button
          type="button"
          onClick={resetForRepeat}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-8">
      <div>
        <label htmlFor="productId" className="block text-sm text-[#655E78] mb-2">
          Product
        </label>
        <select
          id="productId"
      ...
      <div>
        <label htmlFor="customerEmail" className="block text-sm text-[#655E78] mb-2">
          Client&apos;s email
        </label>
        <input
          id="customerEmail"
        ...
        />
        <p className="mt-2 text-xs text-[#655E78]">The finished PDF gets emailed here. Never added to the mailing list.</p>
      </div>

      <div>
        <label htmlFor="client" className="block text-sm text-[#655E78] mb-2">
          What name should we use for them?
        </label>
        <input
          id="client"
          type="text"
      ...
      <div>
        <label className="block text-sm text-[#655E78] mb-2">Focus themes (up to {MAX_THEMES})</label>
      ...
      <div>
        <label className="block text-sm text-[#655E78] mb-2">
          Motivation <span className="text-[#655E78]/60">(optional, up to {MAX_MOTIVATIONS})</span>
        </label>
        <div className="flex flex-wrap gap-2">
      ...
      <div>
        <label className="flex items-center gap-2 text-sm text-[#655E78] mb-3">
          ...
          Client gave specific cities
        </label>
        {!knowsCities && (
          <p className="text-xs text-[#655E78]">Leave this off to rank the best-matching cities algorithmically by theme.</p>
        )}
        {knowsCities && (
      ...
      >
        {isSubmitting ? 'Starting generation...' : 'Generate report'}
      </button>
    </form>
  );
}
