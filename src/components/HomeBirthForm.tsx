  ...
  return (
    <>
        ...
        <div className="max-w-md mx-auto">
          {hasDetails ? (
            <div className="bg-white rounded-2xl border border-[#2D2640]/5 p-6 text-center">
              <p className="text-xs uppercase tracking-widest text-[#7B7394] mb-2">Your details</p>
              <p className="font-serif text-lg text-[#2D2640]">
                ...
                {birthtime && (
                  <span className="text-[#7B7394] font-sans text-sm ml-2">
                    at {birthtime}
                  </span>
                )}
              </p>
              {birthplace && (
                <p className="text-sm text-[#7B7394] mt-1">
                  {birthplace.name}
                </p>
              )}
              <button
                onClick={handleChange}
                className="mt-3 text-xs text-[#7B7394] hover:text-[#2D2640] transition-colors underline underline-offset-2"
              >
                Change details
              </button>
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-[#2D2640]/5 p-6 md:p-8">
              <p className="text-xs uppercase tracking-widest text-[#7B7394] mb-6 text-center">Enter once, explore everything</p>
              <div className="space-y-4">
                <div>
                  <label htmlFor="home-birthdate" className="block text-sm text-[#7B7394] mb-2">Date of birth</label>
                  <input
                    id="home-birthdate"
                    type="date"
                    value={birthdate}
                ...
                <div>
                  <label htmlFor="home-birthtime" className="block text-sm text-[#7B7394] mb-2">Time of birth <span className="text-[#8A8099]">(optional but recommended)</span></label>
                  <input
                    id="home-birthtime"
                    type="time"
                    value={birthtime}
                    ...
                    className="w-full px-4 py-3 border border-[#2D2640]/10 rounded-lg bg-transparent focus:outline-none focus:border-[#2D2640]/30 transition-colors"
                  />
                  <p className="mt-1.5 text-xs text-[#7B7394]">
                    Needed for BaZi, Human Design, and Birth Chart. Other tools work without it.
                  </p>
                </div>
                <div>
                  <label htmlFor="home-birthplace" className="block text-sm text-[#7B7394] mb-2">Place of birth <span className="text-[#8A8099]">(optional but recommended)</span></label>
                  <CitySelect
      ...
      <section className="container-editorial py-12 md:py-16">
        <h2 className="font-serif text-2xl text-[#2D2640] text-center mb-8">Choose a system</h2>
          ...
          {tools.map((tool) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-xs text-[#7B7394] leading-relaxed">
                  {tool.description}
                </p>
                {needsTimeButMissing && (
                  <span className={`mt-2 text-[10px] ${tool.color.text}`}>
        ...
        <div className="mt-8 max-w-3xl mx-auto">
          <p className="text-xs uppercase tracking-widest text-[#7B7394] text-center mb-4">Other tools</p>
          <div className="grid grid-cols-2 gap-4 md:gap-6">
            {otherTools.map((tool) => (
              ...
              >
                ...
                <h3 className="font-serif text-base text-[#2D2640] mb-1">
                  {tool.name}
                </h3>
                <p className="text-xs text-[#7B7394] leading-relaxed">
                  {tool.description}
                </p>
                <span className="mt-2 text-[10px] text-[#8A8099]">
                  {tool.note}
                </span>
              </Link>
            ))}
