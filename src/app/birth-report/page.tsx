...
function BirthReportContent() {
  ...
  const emailModalDismissed = useRef(false);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      let currentSection = cardSections[0].id;
      const viewportTop = 150; // Offset from top to consider "active"

      for (const section of cardSections) {
        const element = document.getElementById(section.id);
        if (element) {
          const rect = element.getBoundingClientRect();
          // If the section's top is above our threshold, it's the current one
  ...
  // Auto-show email modal when user scrolls midway (when floating save appears)
  useEffect(() => {
    if (showFloatingSave && !userEmail && !emailModalDismissed.current && !hasAutoSentEmail.current) {
      setShowEmailModal(true);
      emailModalDismissed.current = true; // Only show once
    }
  }, [showFloatingSave, userEmail]);

  // Auto-scroll mobile nav to show active section pill
  useEffect(() => {
  ...
  useEffect(() => {
    if (showResults && userEmail && !hasAutoSentEmail.current && sunSign) {
  ...
  }, [showResults, userEmail, sunSign, birthMoon, ephemerisChart]);
      ...
      <main className="flex-1">
        {showLoading ? (
          ...
          <>
            ...
            <section className="container-editorial pt-8 md:pt-12 pb-6">
              <div className="flex gap-8">
                ...
                <div className="hidden lg:block w-56 flex-shrink-0">
                  ...
                  <h1 className="font-serif text-3xl text-[#2A2A2A] mb-6">Your Chart</h1>
                ...
                <div ref={reportRef} className="flex-1 min-w-0">
                  ...
                  <div className="lg:hidden mb-6">
                    ...
                    <h1 className="font-serif text-3xl md:text-4xl text-[#2A2A2A]">Your Chart</h1>
                  </div>
            ...
            <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-sm border-t border-[#2A2A2A]/10 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
